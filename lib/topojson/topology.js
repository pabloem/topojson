var type = require("./type"),
    ...
    transformQuantize = require("./transform/codeQuantize.js"),
    bounds = require("./bounds"),
    computeId = require("./compute-id"),
    transformProperties = require("./transform-properties"),
    transformCode = require("./transform/transform.js");
...
module.exports = function(objects, options) {
  ...
  console.log("Skipping prequantization. Topologizing first.");
  /*if (Q0) {
  ...
  if (Q1 || Q0) {
    console.log("Delta encoding after topologizing.");
    delta(topology);
    transform = transformCode.transform(topology,bbox,Q0);
    topology.transform = transform;
    // Post-topology quantization.
    transformQuantize(topology,transform);
    //transform = topology.transform;
    if (verbose) {
      console.warn("post-quantization: " + transform.scale.map(function(k) { return system.formatDistance(k); }).join(" "));
      console.warn("Square Error: "+transform.error.error+" | MSE: "+transform.error.error/transform.error.count+" | Points: "+transform.error.count);
    }
  }
